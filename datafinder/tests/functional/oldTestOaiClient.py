...
class TestOaiClient(unittest.TestCase):
    ...
    def setUp(self, datefile=None):
        ...
        #self.silo = "sandbox"
        self.url = "http://eprints.maths.ox.ac.uk/cgi/oai2"
        self.source = self.url
        ##self.source = self.url+"/"+self.silo
        self.identifiers = []
    ...
    def oai_listIdentifiers(self, src="http://eprints.maths.ox.ac.uk/cgi/oai2", resumptionToken=None):
        self.ids_data_file = '/tmp/%s_ids_data_file'%unicode(uuid.uuid4())
        self.ids_data_file = '/tmp/ids_data_file'
        src_url = None
        if resumptionToken:
            src_url = "%s?verb=ListIdentifiers&resumptionToken=%s"%(src, resumptionToken)
        else:
            src_url = "%s?verb=ListIdentifiers&metadataPrefix=oai_dc"%src
        ...
        self.logger.info("Got the element tree")
        rt = tree.getroot()
        ids = rt.findall("%(ns)sListIdentifiers/%(ns)sheader/%(ns)sidentifier"%{'ns':self.oai_ns})
        self.logger.info(" identifiers for %s"%(ids))
        for ID in ids:
    ...
    def oai_getDCRecord(self, src, identifier, pid):
        ...
        while tries < 11:
            urlretrieve(src_url, self.dc_data_file)
            if os.path.isfile(self.dc_data_file):
                self.logger.info("Downloaded DC for %s --- %s - %s"%(src_url, identifier, pid))
                break
        ...
        metadata_tree.write(self.dc_data_file)
